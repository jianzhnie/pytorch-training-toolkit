...
class TorchTrainer:
    ...
    def forward(self):
        if self._forward is None:
            self._forward = self.optimize(self.forward_fn)
        return self._forward

    @property
    def forward_backward(self):
        self._fwd_bwd = self.optimize(self.fwd_bwd_fn)
    ...
    def validation_steps(self) -> Dict[str, Callable]:
        val_step_dict: Dict[str, Callable] = {"val": self.forward}
    ...
    def train(self, train_loader, val_loader, epochs):
        for epoch in range(epochs):
            ...
            self.eval_mode()
            self.validate_epoch(self.forward, val_loader)
